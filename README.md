...
- `v1_curriculum`：基于 `v0_baseline` 经验重新设计的课程学习版本，目前正在训练中。本轮先提交训练数据、统一 tokenizer 和训练脚本；训练报告、推理报告、checkpoint 和最终结果后续再更新。
...
| Encoder-Decoder Transformer | [v1_curriculum](./encoder_decoder/v1_curriculum/) | 正在训练；已准备分阶段训练数据、统一 48k tokenizer 和训练脚本 |
...
`v1_curriculum` 是基于 `v0_baseline` 暴露的问题重新设计的课程学习版本，目前正在训练中。
...
训练报告、推理报告和最终 checkpoint 会在训练完成并完成复核后再补充。
...
当前 GitHub 更新只计划提交 v1 训练数据、统一 tokenizer 和训练脚本。v1 checkpoint、训练日志、训练报告、推理报告和最终模型分析会在训练完成并复核后再补充。
